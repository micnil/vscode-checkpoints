...
export class CheckpointsController {
  ...
  public initialize() {
    ...
    this.context.subscriptions.push(
      commands.registerCommand(
        ...
        (checkpointNode) => {
          this.promptAreYouSure(
            `Are you sure you want to delete checkpoint '${checkpointNode.label}'?`,
            () => {
      ...
      commands.registerCommand("checkpoints.clearFile", (checkpointNode) => {
        this.promptAreYouSure(
          `Are you sure you want to clear all checkpoints from file '${checkpointNode.nodeId}'?`,
          () => {
            this.model.remove(checkpointNode.nodeId);
          }
        );
      ...
      commands.registerCommand(
        "checkpoints.deselectCheckpoint",
        (checkpointNode) => {
    ...
    this.context.subscriptions.push(
      ...
      commands.registerCommand(
        ...
        this.onAddCheckpoint,
        this
      ),
      commands.registerCommand(
        "checkpoints.diffWithCurrent",
        this.onDiffWithCurrent,
        this
      ),
      commands.registerCommand(
        "checkpoints.diffWithSelection",
        this.onDiffWithSelection,
        this
      ),
      commands.registerCommand(
      ...
      commands.registerCommand(
        "checkpoints.renameCheckpoint",
        this.onRenameCheckpoint,
        this
      ),
      commands.registerCommand(
        "checkpoints.toggleTreeViewContext",
        this.onToggleShowActiveFileOnly,
        this
      )
    );
  }
  ...
  private async onAddCheckpoint() {
    ...
    if (this.activeEditor?.document.uri.scheme === "untitled") {
      console.log(
        `Failed to add file to store. Unsaved documents are currently not supported`
      );
      window.showInformationMessage(
        "Untitled documents are currently not supported"
    ...
    // create default name
    let locale: Maybe<string> = config.get("locale");
  ...
  private async onRestoreCheckpoint(checkpointNode: CheckpointNode) {
    ...
    try {
      ...
      if (success) {
        ...
        if (textDocument.isUntitled) {
          window.showInformationMessage(
            `Restored checkpoint '${checkpointNode.label}'`
          );
        } else {
          window.showInformationMessage(
            `Restored '${textDocument.fileName}' to checkpoint '${checkpointNode.label}'`
          );
          textDocument.save();
        }
      }
      ...
      window.showErrorMessage(
        `Failed to restore file '${checkpointNode.parentId}': ${err.message}`
      );
      console.error(err);
      return;
    }
    ...
    if (
      success &&
      checkpointNode.parentId !== this.model.checkpointContext?.toString()
    ) {
  ...
  private onOpenFile(checkpointNode: CheckpointNode) {
    ...
    workspace.openTextDocument(Uri.parse(checkpointNode.nodeId)).then(
      ...
      (error) => {
        window.showErrorMessage(
          `Cannot open file ${checkpointNode.nodeId}, showing preview of most recent checkpoint instead`
        );
        console.error(error.message);
        let allCheckpoints: ICheckpoint[] = this.model.getCheckpoints(
  ...
  private onRenameCheckpoint(checkpointNode: CheckpointNode) {
    console.log(
      `Rename checkpoint command invoked on checkpoint: '${checkpointNode.label}'`
    );
  ...
  private async onDiffWithCurrent(checkpointNode: CheckpointNode) {
    try {
      if (checkpointNode.parentId) {
        let textDocument = await workspace.openTextDocument(
          Uri.parse(checkpointNode.parentId)
        );

        this.documentView.showDiffWithDocument(
          textDocument.uri,
          checkpointNode.nodeId
        );
      }
      ...
      console.error(err);
      window.showErrorMessage(
        `Failed to show diff for ${checkpointNode.label}: ${err.message}`
      );
  ...
  private onDiffWithSelection(checkpointNode: CheckpointNode): void {
    if (checkpointNode.parentId) {
      ...
      if (file.selection === "") {
        window.showInformationMessage(
          "Please mark a checkpoint as selected first."
        );
        return;
      }

      this.documentView.showDiffWithCheckpoint(
        checkpointNode.nodeId,
        file.selection
      );
    }
    ...
    config.update("showActiveFileOnly", !currentConfigValue).then(
      () => {
        window.setStatusBarMessage(
          `Set showActiveFileOnly config to '${!currentConfigValue}'`,
          5000
        );
        this.treeView.refresh();
      },
  ...
  private async openTextDocument(checkpoint: CheckpointNode) {
    try {
      ...
      window.showWarningMessage(
        "Failed to open original document, opening untitled document instead."
      );
  ...
  private handleOnSave() {
    const config = workspace.getConfiguration("checkpoints");
    const addCheckpointOnSave: Maybe<boolean> = config.get(
      "addCheckpointOnSave"
    );
