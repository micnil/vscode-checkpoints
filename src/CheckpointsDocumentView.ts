...
export class CheckpointsDocumentView implements TextDocumentContentProvider {
  ...
  constructor(context: ExtensionContext, private model: CheckpointsModel) {
    ...
    context.subscriptions.push(
      model.onDidRemoveCheckpoint(
        (removedItem: ICheckpoint | IFile | ICheckpointStore) => {
   ...
   */
  public showDiffWithDocument(
    comparisonDocumentUri: Uri,
    checkpointId: string
  ) {
    console.log(`
  ...
  public showDiffWithCheckpoint(
    ...
    if (!checkpoint1) {
      console.error(
        `The checkpoint with id: '${checkpointId1}' does not exist`
      );
      return;
    } else if (!checkpoint2) {
      console.error(
        `The checkpoint with id: '${checkpointId2}' does not exist`
      );
      return;
    }
   ...
   */
  public provideTextDocumentContent(uri: Uri): Maybe<string> {
  ...
  private getCheckpointUri(checkpoint: ICheckpoint): Uri {
    ...
    // files path
    return Uri.parse(
      `checkpointsDocumentView://checkpoint${filePath.path}#${checkpoint.id}`
    );
  }
}
